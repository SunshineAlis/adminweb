    ...
    const fetchCategories = async () => {
        try {
            ...
            const categoriesWithFoodCount = await Promise.all(response.data.map(async (cat: any) => {
                ...
                const foodResponse = await axios.get(`http://localhost:4040/category/${cat._id}/foods`);
                console.log("Food count response:", foodResponse.data);
                return { ...cat, foodCount: foodResponse.data.foodCount };
            }));
            ...
            <div className="max-w-[800px] w-[100%] m-auto flex flex-wrap gap-3 items-center">
                {categories.map((cat) => (
                    <div key={cat._id} className="relative dropdown-container">
                        ...
                        >
                            {cat.categoryName} ({cat.foodCount || 0}) {/* 显示 foodCount */}
                        </span>
